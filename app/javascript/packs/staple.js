...
function getArrowHead(tipPosition, dir, distance) {
  ...
  arrowHeadGeometry.setAttribute(
    "position",
    new THREE.Float32BufferAttribute(arrowHeadPosition, 3)
  );
...
function getArrowTail(basePosition) {
  ...
  const arrowTailMaterial = new THREE.MeshBasicMaterial({
    color: 0x000000,
    side: THREE.DoubleSide,
  });
  const arrowTailMesh = new THREE.Mesh(arrowTailGeometry, arrowTailMaterial);
  arrowTailMesh.position.set(
    basePosition.x,
    basePosition.y + 0.01,
    basePosition.z
  );
